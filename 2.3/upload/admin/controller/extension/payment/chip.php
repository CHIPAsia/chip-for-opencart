...
class ControllerExtensionPaymentChip extends Controller {
  ...
  public function index() {
    ...
    $data['breadcrumbs'][] = array(
      ...
      'href'      => $this->url->link('extension/payment/chip', 'token=' . $this->session->data['token'], true),
    );

    $data['action'] = $this->url->link('extension/payment/chip', 'token=' . $this->session->data['token'], true);
  ...
  private function configure_general_public_key() {
    $this->load->model('extension/payment/chip');
    $this->model_extension_payment_chip->set_keys($this->request->post['chip_secret_key'], '');
